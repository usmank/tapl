...
typeOf (TmIf sp t1 t2 t3) ctx =
  ...
  in  if t1Type == TyBool
        ...
        else err sp "Type error: Conditional guard is not a boolean."
typeOf (TmVar sp i) ctx = fromMaybe (err sp "Type error: Variable type lookup failed.") (getVarType i ctx)
